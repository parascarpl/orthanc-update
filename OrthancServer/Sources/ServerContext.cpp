...
namespace Orthanc
{
                                                                  ...
                                                                  bool isReconstruct)
  {
    ...
    try
    {
      ...
      instance_ = hasher.GetStudyUid();
      instance_ += "\\" + hasher.GetSeriesUid();
      instance_ += "\\" + hasher.GetInstanceUid();


      FileInfo dicomInfo = accessor.Write(dicom.GetBufferData(), dicom.GetBufferSize(), 
                                          FileContentType_Dicom, compression, storeMD5_, instance_);
           ...
           compressionEnabled_))
      {
        dicomUntilPixelData = accessor.Write(dicom.GetBufferData(), pixelDataOffset, 
                                             FileContentType_DicomUntilPixelData, compression, storeMD5_, instance_);
                                                  ...
                                                  CompressionType compression)
  {
    ...
    FileInfo modified = accessor.Write(content.empty() ? NULL : content.c_str(),
                                       content.size(), attachmentType, compression, storeMD5_, instance_);
                                    ...
                                    const std::string& oldMD5)
  {
    ...
    StorageAccessor accessor(area_, &storageCache_, GetMetricsRegistry());
    FileInfo attachment = accessor.Write(data, size, attachmentType, compression, storeMD5_, instance_);
